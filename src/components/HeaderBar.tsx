...
const HeaderBar: React.FC<Props> = ({ onOpenFilters, onOpenProfile }) => {
  return (
    <div className="sticky top-0 z-50 border-b" style={{ background: 'var(--accent-bg)', color: 'var(--accent-text)', borderColor: 'rgba(255,255,255,0.2)' }}>
      <div className="max-w-7xl mx-auto px-6 sm:px-9 py-3">
        <div className="flex items-center gap-3">
          <button className="w-10 h-10 rounded-lg border flex items-center justify-center" style={{ background: 'var(--control-bg)', borderColor: 'var(--control-border)' }} onClick={onOpenFilters} aria-label="Open filters">
          ...
          <button className="w-12 h-12 rounded-full flex items-center justify-center border" style={{ background: 'var(--control-bg)', borderColor: 'var(--control-border)' }} aria-label="Profile" onClick={onOpenProfile}>
